  ...
  if [ "$host" = "local" ]; then
    if [ "$DSPARK_BUILD_STAGE" = "anemll-416" ]; then
      ...
      docker run --rm --entrypoint python3 "$DSPARK_VLLM_IMAGE" -c \
        "from pathlib import Path; from vllm.models.deepseek_v4.nvidia.nvfp4_cache import RECORD_BYTES; source = Path('/usr/local/lib/python3.12/dist-packages/vllm/models/deepseek_v4/nvidia/dspark.py').read_text(); assert RECORD_BYTES == 416; assert 'attn.kv_cache_dtype == \"nvfp4_ds_mla\"' in source; assert 'rope_store_swa_nvfp4_416' in source; env_source = Path('/usr/local/lib/python3.12/dist-packages/vllm/envs.py').read_text(); core_source = Path('/usr/local/lib/python3.12/dist-packages/vllm/v1/engine/core.py').read_text(); scheduler_source = Path('/usr/local/lib/python3.12/dist-packages/vllm/v1/core/sched/scheduler.py').read_text(); assert 'VLLM_PREFILL_DECODE_CADENCE' in env_source; assert 'Interactive prefill/decode cadence enabled' in core_source; assert '_get_prefill_token_budget' in scheduler_source; assert '_get_prefill_token_threshold' in scheduler_source; print('dspark anemll nvfp4-416 image ok')"
      return
    fi
    ...
    ssh "$host" "cd '$checkout' && ENV_FILE=/dev/null DSPARK_BASE_IMAGE='$DSPARK_BASE_IMAGE' DSPARK_VLLM_IMAGE='$DSPARK_VLLM_IMAGE' DSPARK_BUILD_STAGE='$DSPARK_BUILD_STAGE' DSPARK_STAGE_C_IMAGE='$DSPARK_STAGE_C_IMAGE' DSPARK_ANEMLL_BASE_IMAGE='$DSPARK_ANEMLL_BASE_IMAGE' WORKER_BUILD=0 ./build-dspark-vllm-runtime.sh"
  fi
}
