...
def main() -> None:
    ...
    lengths = torch.tensor([topk - 3], dtype=torch.int32, device=device)
    ...
    torch.testing.assert_close(
        native_output, direct_output, rtol=0.04, atol=0.03125
    )
    ...
    torch.testing.assert_close(
        dual_native_output, dual_direct_output, rtol=0.04, atol=0.03125
    )
