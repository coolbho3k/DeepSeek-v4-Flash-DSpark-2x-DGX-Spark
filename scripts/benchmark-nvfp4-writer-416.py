#!/usr/bin/env python3
"""Benchmark DSpark's SWA, C4, and C128 416-byte cache writers."""
...
def main() -> None:
    ...
    parser.add_argument("--c128-rows", type=int, nargs="+", default=[])
    parser.add_argument("--c128-decode-rows", type=int, nargs="+", default=[])
    ...
    if not (args.c4_rows or args.c128_rows or args.c128_decode_rows):
        return
    ...
    if not (args.c128_rows or args.c128_decode_rows):
        return

    print(
        "\nC128 mode       rows  two_stage_ms  fused_ms  speedup  "
        "graph_two_ms  graph_fused_ms  graph_speedup  max_error"
    )
    c128_cases = [
        *(("prefill", rows) for rows in args.c128_rows),
        *(("decode-hit", rows) for rows in args.c128_decode_rows),
        *(("decode-idle", rows) for rows in args.c128_decode_rows),
    ]
    for mode, rows in c128_cases:
        state_block_size = 8
        state_width = HEAD_DIM
        if mode == "prefill":
            if rows < 128:
                parser.error("--c128-rows values must be at least 128")
            state_blocks = (rows + state_block_size - 1) // state_block_size
            positions = torch.arange(rows, dtype=torch.int64, device=device)
            state_slots = positions.clone()
            ...
            valid = (positions + 1) % 128 == 0
            kv_slots = torch.where(valid, positions // 128, -1)
            kv_rows = max(1, rows // 128)
            angle_rows = rows
        else:
            if rows < 1:
                parser.error("--c128-decode-rows values must be at least 1")
            blocks_per_request = 128 // state_block_size
            state_blocks = rows * blocks_per_request
            positions = torch.full(
                (rows,),
                127 if mode == "decode-hit" else 126,
                dtype=torch.int64,
                device=device,
            )
            state_slots = torch.arange(rows, dtype=torch.int64, device=device)
            request_indices = torch.arange(
                rows, dtype=torch.int32, device=device
            )
            block_table = torch.arange(
                state_blocks, dtype=torch.int32, device=device
            ).view(rows, blocks_per_request)
            kv_slots = (
                torch.arange(rows, dtype=torch.int64, device=device)
                if mode == "decode-hit"
                else torch.full((rows,), -1, dtype=torch.int64, device=device)
            )
            kv_rows = rows
            angle_rows = 128
        state_cache = torch.randn(
        ...
        angles = torch.randn(
            (angle_rows, ROPE_DIM // 2), generator=generator, device=device
        )
        cos_sin_cache = torch.cat((angles.cos(), angles.sin()), dim=-1)
        metadata = SimpleNamespace(slot_mapping=kv_slots)
        ...
        fused()
        torch.cuda.synchronize()
        two_stage_values = unpack_reference(two_stage_cache, rows=kv_rows)
        fused_values = unpack_reference(fused_cache, rows=kv_rows)
        error = (fused_values - two_stage_values).abs()
        torch.testing.assert_close(
            fused_values, two_stage_values, rtol=0.04, atol=0.04
        )

        two_stage_graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(two_stage_graph):
            two_stage()
        fused_graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(fused_graph):
            fused()
        graph_allocated = torch.cuda.memory_allocated()
        fused_graph.replay()
        torch.cuda.synchronize()
        if torch.cuda.memory_allocated() != graph_allocated:
            raise AssertionError("fused C128 graph replay changed CUDA allocation")
        ...
        graph_two_stage_ms = measure_ms(
            two_stage_graph.replay,
            warmup=args.warmup,
            repetitions=args.repetitions,
        )
        graph_fused_ms = measure_ms(
            fused_graph.replay,
            warmup=args.warmup,
            repetitions=args.repetitions,
        )
        print(
            f"{mode:11s}  {rows:7d}  {two_stage_ms:12.4f}  {fused_ms:8.4f}  "
            f"{two_stage_ms / fused_ms:7.2f}x  {graph_two_stage_ms:12.4f}  "
            f"{graph_fused_ms:14.4f}  "
            f"{graph_two_stage_ms / graph_fused_ms:11.2f}x  "
            f"{error.max().item():9.6f}"
        )
