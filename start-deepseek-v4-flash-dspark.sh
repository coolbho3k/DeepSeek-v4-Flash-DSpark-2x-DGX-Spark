...
VLLM_PREFILL_DECODE_CADENCE="${VLLM_PREFILL_DECODE_CADENCE:-16}"
if ! [[ "$VLLM_PREFILL_DECODE_CADENCE" =~ ^[0-9]+$ ]] \
  || (( 10#$VLLM_PREFILL_DECODE_CADENCE < 1 )); then
  echo "VLLM_PREFILL_DECODE_CADENCE must be an integer >= 1: $VLLM_PREFILL_DECODE_CADENCE" >&2
  exit 2
fi
VLLM_PREFILL_DECODE_CADENCE="$((10#$VLLM_PREFILL_DECODE_CADENCE))"
...
export GPU_MEMORY_UTILIZATION LONG_PREFILL_TOKEN_THRESHOLD VLLM_PREFILL_DECODE_CADENCE SCHEDULING_POLICY
...
case "$VLLM_DSV4_DEMAND_SIZED_KV_POOLS" in
  0|1) ;;
  *)
    echo "VLLM_DSV4_DEMAND_SIZED_KV_POOLS must be 0 or 1 (got: $VLLM_DSV4_DEMAND_SIZED_KV_POOLS)" >&2
    exit 2
    ;;
esac
export VLLM_HOST VLLM_PORT PORT DEFAULT_THINKING USE_FP4_INDEXER_CACHE VLLM_DSV4_DEMAND_SIZED_KV_POOLS
...
REMOTE_PREFILL_DECODE_CADENCE="$(printf '%q' "$VLLM_PREFILL_DECODE_CADENCE")"
REMOTE_SCHEDULING_POLICY="$(printf '%q' "$SCHEDULING_POLICY")"
REMOTE_COMPOSE="cd $REMOTE_WORKER_DIR && env -u MASTER_ADDR -u MASTER_PORT -u NODE_RANK -u HEADLESS COMPOSE_DISABLE_ENV_FILE=1 GPU_MEMORY_UTILIZATION=$REMOTE_GPU_MEMORY_UTILIZATION LONG_PREFILL_TOKEN_THRESHOLD=$REMOTE_LONG_PREFILL_TOKEN_THRESHOLD VLLM_PREFILL_DECODE_CADENCE=$REMOTE_PREFILL_DECODE_CADENCE SCHEDULING_POLICY=$REMOTE_SCHEDULING_POLICY"
STARTUP_LOG_SINCE=""
...
print_resolved_profile() {
  ...
  echo "  mixed prefill token threshold: $LONG_PREFILL_TOKEN_THRESHOLD (0 disables; pure prefill uses a fair budget share)"
